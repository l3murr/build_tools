...
RUN mkdir -p /etc/apt/keyrings && \
    ...
    apt-get update && \
    apt-get install -y nodejs git unzip net-tools locales supervisor clang mysql-client libz-dev libssl-dev libmysqlclient-dev pkg-config cmake nginx && \
...
if [ -f /var/www/html/README.md ]; then\n\
  ...
  /root/.cargo/bin/cargo install diesel_cli\n\
