...
RUN mkdir -p /etc/apt/keyrings && \
    ...
    apt-get update && \
    apt-get install -y nodejs git unzip net-tools locales supervisor clang mysql-client libz-dev libssl-dev libmysqlclient-dev protobuf-compiler pkg-config cmake nginx zsh zsh-autosuggestions zsh-syntax-highlighting vim libwebkit2gtk-4.1-dev build-essential wget file libxdo-dev libssl-dev libayatana-appindicator3-dev librsvg2-dev xdg-utils && \
